 ...
 */
var Launchpad = function Launchpad(port, initAnimation) {
    ...
    this.getButton = function (x, y) {
        ...
        if (y !== undefined) return this._grid[x][y];
    ...
    this.receiveMessage = function (deltaTime, msg) {
        ...
        if (msg[0] == "176") button = that.getButton(parseInt(msg[1], 10) % 8, 8);
    ...
    this.init = function () {
        ...
        if (port !== undefined) {
            ...
            var openLaunchpadPort = function openLaunchpadPort(io, name) {
                return new Array(io.getPortCount()).fill(0).map(function (el, ix) {
                    return { name: io.getPortName(ix), port: ix };
                }).filter(function (desc) {
                    return desc.name.indexOf('Launchpad') >= 0;
                }).some(function (desc) {
            ...
            var openedOutput = openLaunchpadPort(this.output, 'output'),
...
Launchpad.prototype.colorize = function (bytes, colorStr, nullColorStr) {
    for (var i = 0; i < bytes.length; i++) {
        ...
        for (var j = 0; j < bytes[i].length; j++) {
            if (bytes[i][j] === "1") str += colorStr;else str += nullColorStr;
        }
        bytes[i] = str;
    }
    return bytes;
...
Launchpad.prototype.scrollBytes = function (bytes, delay, color, onFinished) {
    ...
    interval = setInterval(function () {
        var set = visibleSet();
        if (set === undefined && typeof onFinished == "function") onFinished();else that.renderBytes(set, color);
    }, delay);
};
